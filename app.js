...
// --- MONETAG AD ZONES ---
// Prioritize based on assumed CPM. Fallback if one fails.
...
// --- CORE: REWARD SYSTEM ---
const updateBalance = async (amount, message = "Ad Reward") => {
...
// Sync Balance to UI (small and large displays)
...
// --- AD HANDLERS ---
// Function to show ads in a prioritized order
async function showPrioritizedAd(rewardAmount, rewardMessage) {
    ...
    for (const adFunction of MONETAG_AD_FUNCTIONS) {
        try {
            await adFunction();
            adShown = true;
            break; // Stop after first successful ad
        } catch (e) {
            console.warn(`Monetag Ad failed for zone ${adFunction.name || 'unknown'}:`, e);
        }
    }
    Swal.close();
    if (adShown) {
...
window.showPrioritizedAd = showPrioritizedAd; // Make it globally accessible

// --- CHAT SYSTEM ---
const sendBtn = document.getElementById('send-btn');
const chatInput = document.getElementById('chat-input');
const cooldownTimerDisplay = document.getElementById('cooldown-timer');
let lastMsgTime = 0; // In milliseconds
...
sendBtn.onclick = async () => {
    const now = Date.now();
    if (now - lastMsgTime < CHAT_COOLDOWN_MS) {
    ...
    try {
        await MONETAG_AD_FUNCTIONS[0](); // Ad 1
        await MONETAG_AD_FUNCTIONS[1](); // Ad 2
        await MONETAG_AD_FUNCTIONS[2](); // Ad 3
    } catch (e) {
        allAdsShown = false;
        Swal.close();
...
onValue(query(ref(db, 'messages'), limitToLast(20)), (snapshot) => {
    ...
    messages.forEach(msg => {
        ...
        msgElement.innerHTML = `
            <p class="text-[9px] text-green-300 font-bold uppercase mb-1">${msg.name}</p>
...
window.adminLogin = () => {
    Swal.fire({
        ...
        if (result.value === ADMIN_PASSWORD) {
            ...
            loadAdminPanel();
        } else if(result.value) { // User entered something but it was wrong
...
const loadAdminPanel = () => {
    ...
    onValue(query(ref(db, 'withdrawals'), orderByChild('timestamp')), (snapshot) => {
        ...
        snapshot.forEach(child => {
            ...
            if (withdrawal.status === 'pending') { // Only show pending requests
                allWithdrawals.push({ key: child.key, ...withdrawal });
            }
        });
        allWithdrawals.reverse(); // Show newest pending first
        currentPage = 0; // Reset to first page on new data
        renderAdminTable();
        renderPagination();
    });
};
...
tg.ready();
tg.expand();
updateCooldownDisplay(); // Start cooldown timer on load
